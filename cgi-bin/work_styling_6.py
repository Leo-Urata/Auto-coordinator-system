  ...
  <body>
    ...
    <div class="container bg-light">
        ...
        <table class="table">
          ...
          <tbody>
            <tr>
              <td><img src="../styling_outer_img/out6.jpg" class="img-fluid"></td>
              <td><img src="../styling_tops_img/top6.jpg" class="img-fluid"></td>
              <td><img src="../styling_buttoms_img/but6.jpg" class="img-fluid"></td>
            </tr>
          </tbody>
        </table>
        
        <form action="work_styling_6.py" method="post" enctype="multipart/form-data">
...
#    cgi用パス
    filepath_outer='./styling_outer_img/out6.jpg'
    filepath_tops='./styling_tops_img/top6.jpg'
    filepath_buttoms='./styling_buttoms_img/but6.jpg'
...
#    クエリベクトル生成機能
    ...
    qvec_outer=ext_mean_rgb(filepath_outer)
    qvec_tops=ext_mean_rgb(filepath_tops)
    qvec_buttoms=ext_mean_rgb(filepath_buttoms)

#    meta.csv読み込み機能
    df_outer = pd.read_csv('./cgi-bin/metadata/metadata_outer.csv', index_col=0)
    tfilepaths_outer=df_outer['imgname']
    rgbs_outer=df_outer[['R','G','B']]
...
#    qvecとコサイン類似度を計算する
    ...
    result_outer=np.array([])
    result_tops=np.array([])
    result_buttoms=np.array([])
    
    for i in range(rgbs_outer.index.shape[0]):
      result_outer=np.append(result_outer, comp_sim(qvec_outer, rgbs_outer.iloc[i,:].to_numpy()))
...
#    類似度が大きい順にソートして表示
    rank_outer=np.argsort(result_outer)
    rank_tops=np.argsort(result_tops)
    rank_buttoms=np.argsort(result_buttoms)
    
    for index in rank_outer[:-rank_outer.shape[0]+1:-10000]:
        title_outer.append('{}'.format(tfilepaths_outer[index]))
    
    for index in rank_outer[:-rank_outer.shape[0]+1:-10000]:
        score_outer.append('{}'.format(result_outer[index]))
    ...
    for index in rank_outer[:-rank_outer.shape[0]+1:-10000]:
        contents1=contents1+'<tr><td><img src=".'+tfilepaths_outer[index]+'" width="25%" hight="auto"></td><td>'+str(result_outer[index])+'</td></tr>'
